...
void MyRobot::run()
{
    ...
    while (step(_time_step) != -1) {
		...
		cout << "Backing up and turning right." << endl;
                ...
                if((ir12_val > ir13_val)||(ir9_val > DISTANCE_LIMIT)||(ir10_val > DISTANCE_LIMIT)||(ir8_val > DISTANCE_LIMIT) ||   				(ir10_val<ir11_val)){
            	   _mode = TURN_LEFT;
                   cout << "TURN LEFT." << endl;
                  }
		if(((ir3_val|| ir4_val) > ir2_val)|| ((ir12_val || ir11_val) > (ir13_val || ir14_val))
		||(ir6_val > DISTANCE_LIMIT)||(ir5_val > DISTANCE_LIMIT)||(ir7_val > DISTANCE_LIMIT)|| (ir4_val>ir5_val)){
                   _mode = TURN_RIGHT;
                   cout << "TURN RIGHT" << endl;
                   }
		/*if ((ir9_val > DISTANCE_LIMIT)||(ir10_val > DISTANCE_LIMIT) || (ir10_val<ir11_val)) {
           	    _mode = TURN_LEFT;
                    cout << "TURN LEFT" << endl;
        	}
		if ((ir6_val > DISTANCE_LIMIT)||(ir5_val > DISTANCE_LIMIT)|| (ir4_val>ir5_val)) {
		     _mode = TURN_RIGHT;
	             cout << "TURN RIGHT" << endl;
			}
                */
